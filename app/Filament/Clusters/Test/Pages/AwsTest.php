...
class AwsTest extends XotBasePage
{
    ...
    protected function getAwsConfig(): array
    {
        $region = config('filesystems.disks.s3.region');
        $bucket = config('filesystems.disks.s3.bucket');
        $cloudFrontUrl = config('filesystems.cloudfront.url');
        $cloudFrontKeyPairId = config('filesystems.cloudfront.key_pair_id');

        return [
            'AWS_ACCESS_KEY_ID' => substr((string) config('filesystems.disks.s3.key', ''), 0, self::KEY_PREVIEW_LENGTH).'...',
            'AWS_DEFAULT_REGION' => is_string($region) ? $region : null,
            'AWS_BUCKET' => is_string($bucket) ? $bucket : null,
            'CLOUDFRONT_URL' => is_string($cloudFrontUrl) ? $cloudFrontUrl : null,
            'CLOUDFRONT_KEY_PAIR_ID' => is_string($cloudFrontKeyPairId) ? $cloudFrontKeyPairId : null,
        ];
    }
